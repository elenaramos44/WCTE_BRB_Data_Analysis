...
# --- Stable NLL for double Gaussian mixture ---
...
def fit_double_gauss_multistart(data, n_starts=12, verbose=False):
    best = None
    best_nll = np.inf
    ...
    for p0 in init_list:
        res = minimize(stable_nll, p0, args=(data,), method="L-BFGS-B", bounds=bounds, options={'maxiter':1000})
    ...
    return {'mu1':mu1f,'sigma1':s1f,'mu2':mu2f,'sigma2':s2f,'w':wf,'nll':best.fun,'success':best.success,'result_obj':best}

# --- Loop over PMTs ---
results_list = []

#for pmt_label in tqdm(pmts_both[:10], desc="Processing PMTs"):
for pmt_label in pmts_both:
    ...
    # Fit pedestal (component with smaller mean)
    fit_ped = fit_double_gauss_multistart(charges_control, n_starts=10, verbose=False)
    mu_ped, sigma_ped = fit_ped['mu1'], fit_ped['sigma1']
    
    # Fit SPE (component with larger mean)
    fit_spe = fit_double_gauss_multistart(charges_signal, n_starts=15, verbose=False)
    mu_spe, sigma_spe = fit_spe['mu2'], fit_spe['sigma2']
    
    # Gain & error
    gain = mu_spe - mu_ped
    err_gain = np.sqrt( (sigma_ped/np.sqrt(len(charges_control)))**2 + (sigma_spe/np.sqrt(len(charges_signal)))**2 )
...
npz_file_out = "/scratch/elena/WCTE_2025_commissioning/2025_data/WCTE_BRB_Data_Analysis/pmt_charge_fit_results_doubleGauss.npz"
